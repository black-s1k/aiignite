...
export function Spark() {
  return (
    <Sheet id="spark" slug="Spark · sessions" press="spark">
      ...
      <ul className="mt-16 border-t border-chalk/25">
        {SPARK_SESSIONS.map((s, i) => (
          <Reveal
            as="li"
            key={s.title}
            delay={i * 55}
            className="border-b border-chalk/25 py-7"
          >
            <div className="flex flex-wrap items-baseline justify-between gap-x-6 gap-y-2">
              <h3 className="font-display text-2xl font-extrabold uppercase leading-none tracking-tight text-chalk sm:text-3xl">
                {s.title}
              </h3>
            ...
            <p className="mt-4 flex items-baseline gap-3 font-mono text-xs text-muted">
              {/* The mark, not the word. The flame could carry text on
                  black stock, but a green sentence beside a green
                  sentence beside a green sentence is noise — it points
                  instead, and the line stays in one voice. */}
